...
export function formatCharacterPersona(character) {
    ...
    return [
        ...
        `- 말투 스타일: ${toDisplayText(character.speechStyles, 'speechStyles')}`,
        `- 기본 행동 규칙: ${toDisplayText(character.behaviorRules, 'behaviorRules')}`,
        `- 기타 행동 규칙: ${customBehaviorText}`,
        `- 좋아하는 것: ${toDisplayText(character.likes, 'likes')}`,
        `- 싫어하는 것: ${toDisplayText(character.dislikes, 'dislikes')}`,
        `- 싫어하는 것 직접 입력: ${customDislikesText}`,
        `- 사용자와 관계: ${OPTION_LOOKUPS.relationship.get(character.relationship) || '친구'}`,
        `- 캐릭터 목표: ${toDisplayText(character.goals, 'goals')}`,
        `- 캐릭터 목표 직접 입력: ${customGoalsText}`,
        `- 캐릭터 배경: ${character.background || '미설정'}`,
    ].join('\n');
}
